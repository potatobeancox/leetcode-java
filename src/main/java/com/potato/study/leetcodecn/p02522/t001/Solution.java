package com.potato.study.leetcodecn.p02522.t001;
...
public class Solution {
    ...
    public int minimumPartition(String s, int k) {
        ...
        while (right < s.length()) {
            //  当前数字
            temp *= 10;
            if (s.charAt(right) - '0' > k) {
                return -1;
            }
            temp += (s.charAt(right) - '0');
            if (temp > k) {
                partCount++;
                temp %= 10;
            }
    ...
    public static void main(String[] args) {
        ...
        String s = "165462";
        int k = 60;
        int i = solution.minimumPartition(s, k);
        System.out.println(i);
        Assert.assertEquals(4, i);

        s = "1";
        k = 1;
        i = solution.minimumPartition(s, k);
        System.out.println(i);
