package com.potato.study.leetcodecn.p02194.t001;
...
public class Solution {
    ...
    public List<String> cellsInRange(String s) {
        ...
        // 开始字母 终止字母 开始 数字 终止数字
        char ch = split[0].charAt(0);
        int startIndex = split[0].charAt(1) - '0';

        char endCh = split[1].charAt(0);
        int endIndex = split[1].charAt(1) - '0';

        List<String> result = new ArrayList<>();
        for (int i = ch - 'A'; i <= endCh - 'A'; i++) {
            for (int j = startIndex; j <= endIndex; j++) {
                String target = "" + (char)('A' + i) + j;
                result.add(target);
            }
        }
        return result;
