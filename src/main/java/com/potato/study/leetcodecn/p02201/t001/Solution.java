package com.potato.study.leetcodecn.p02201.t001;
...
public class Solution {
    ...
    public int digArtifacts(int n, int[][] artifacts, int[][] dig) {
        ...
        for (int i = 0; i < dig.length; i++) {
            status[dig[i][0]][dig[i][1]] = true;
        }
        // 遍历 每个 artifacts 看看 dig 对应位置 是不是都是 true
        int count = 0;
        for (int i = 0; i < artifacts.length; i++) {
            boolean hasDig = true;
            int startI = artifacts[i][0];
            int startJ = artifacts[i][1];
            int endI = artifacts[i][2];
            int endJ = artifacts[i][3];
