package com.potato.study.leetcodecn.p02034.t001;
...
public class StockPrice {
    ...
    public StockPrice() {
        ...
        // treemap 存当前价格和出现次数 用于 max min 操作
        this.countMap = new TreeMap<>();
    }

    public void update(int timestamp, int price) {
        // 更新最新时间戳
        this.currtent = Math.max(currtent, timestamp);
        // 如果 hash map 存在这个时间戳 更新价格 并更新 treemap
        if (timePriceMap.containsKey(timestamp)) {
            int oldPrice = timePriceMap.get(timestamp);
            // 老的价格维护
            Integer oldCount = countMap.get(oldPrice);
            oldCount--;
            if (oldCount == 0) {
                countMap.remove(oldPrice);
            } else {
                countMap.put(oldPrice, oldCount);
            }
            // update 新的 字段
    ...
    public int current() {
        return timePriceMap.get(this.currtent);
    }
