package com.potato.study.leetcodecn.other.lcp.p0072.t001;
...
public class Solution {
    ...
    public int[] supplyWagon(int[] supplies) {
        ...
        while (len > supplies.length / 2) {
            ...
            for (int i = 0; i < len - 1; i++) {
                if (supplies[minStartIndex] + supplies[minStartIndex+1] > supplies[i] + supplies[i+1]) {
                    minStartIndex = i;
                }
            }
            // 合并 min min+ 1
            supplies[minStartIndex] += supplies[minStartIndex + 1];
            // 将min+ 2 开始往前移动
            for (int i = minStartIndex + 2; i < len; i++) {
                supplies[i-1] = supplies[i];
            }
            len--;
        }
        int[] res = new int[len];
        System.arraycopy(supplies, 0, res, 0, len);
        return res;
    }
    ...
    public static void main(String[] args) {
        ...
        int[] ints = solution.supplyWagon(supplies);
