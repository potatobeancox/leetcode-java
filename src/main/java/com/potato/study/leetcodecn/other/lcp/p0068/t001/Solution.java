package com.potato.study.leetcodecn.other.lcp.p0068.t001;
...
public class Solution {
    ...
    public int beautifulBouquet(int[] flowers, int cnt) {
        // 用一个窗口记录当前是不是窗口满足 都小于 cnt 小于的话 加上窗口长度
        Map<Integer, Integer> countMap = new HashMap<>();
        int left = 0;
        long result = 0;
        int mod = 1_000_000_000 + 7;
        for (int i = 0; i < flowers.length; i++) {
            int count = countMap.getOrDefault(flowers[i], 0);
            count++;
            countMap.put(flowers[i], count);
            while (count > cnt) {
                Integer leftCount = countMap.get(flowers[left]);
                leftCount--;
                countMap.put(flowers[left], leftCount);

                count = countMap.get(flowers[i]);
            }
            // 当前窗口都满足 那么窗口大小
            int window = i - left + 1;
            if (window > 0) {
