package com.potato.study.leetcodecn.p00609.t001;
    ...
    public List<List<String>> findDuplicate(String[] paths) {
        ...
        for (int i = 0; i < paths.length; i++) {
            ...
            for (int j = 1; j < split.length; j++) {
                ...
                String content = split[j].substring(index+1, split[j].length() - 1);

                List<String> resultList = contentMap.getOrDefault(content, new ArrayList<>());
                resultList.add(parentPath + "/" + split[j].substring(0, index));

                contentMap.put(content, resultList);
            }
        }
        // 收集结果
        List<List<String>> totalList = new ArrayList<>();
        for (List<String> each : contentMap.values()) {
            if (each.size() > 1) {
                totalList.add(each);
            }
        }
