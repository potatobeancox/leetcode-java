package com.potato.study.leetcodecn.p02190.t001;
...
public class Solution {
    ...
    public int mostFrequent(int[] nums, int key) {
        ...
        for (int i = 0; i < nums.length - 1; i++) {
            if (nums[i] == key) {
                Integer count = countMap.getOrDefault(nums[i + 1], 0);
                count++;
                countMap.put(nums[i+1], count);
                if (maxValue == 0
                        || count > countMap.get(maxValue)) {
                    maxValue = nums[i+1];
                }
            }
        }
    ...
    public static void main(String[] args) {
        ...
        int key = 1;
        int i = solution.mostFrequent(arr, key);
