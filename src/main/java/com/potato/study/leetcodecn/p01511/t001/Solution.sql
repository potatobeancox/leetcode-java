...
-- 题解
-- https://leetcode.cn/problems/customer-order-frequency/solution/ms-sql-bu-shi-yong-lin-shi-biao-zhi-yong-group-byh/
-- left 函数 字符串的左部分
...
HAVING sum(
  CASE WHEN LEFT(Orders.order_date, 7) = '2020-06' THEN Orders.quantity * Product.price ELSE 0 END
) >= 100
and sum(
  CASE WHEN LEFT(Orders.order_date, 7) = '2020-07' THEN Orders.quantity * Product.price ELSE 0 END
) >= 100
