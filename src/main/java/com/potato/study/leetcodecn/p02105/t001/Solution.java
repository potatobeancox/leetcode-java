package com.potato.study.leetcodecn.p02105.t001;
...
public class Solution {
    ...
    public int minimumRefill(int[] plants, int capacityA, int capacityB) {
        ...
        int n = plants.length;
        int right = n - 1;
        int remindA = capacityA;
        int remindB = capacityB;
        int returnCount = 0;
        while (left <= right) {
            // 如果相遇 多的那个浇水
            if (left == right) {
                int maxRemind = Math.max(remindA, remindB);
                if (maxRemind < plants[left]) {
                    returnCount ++;
                }
                break;
            } else {
                if (remindA < plants[left]) {
                    returnCount++;
                    remindA = capacityA;
                }
                if (remindB < plants[right]) {
                    returnCount++;
                    remindB = capacityB;
                }
                remindA -= plants[left];
                remindB -= plants[right];
                left++;
                right--;
            }
        }
    ...
    public static void main(String[] args) {
        ...
        int capacityA = 5;
        int capacityB = 5;
        int i = solution.minimumRefill(plants, capacityA, capacityB);
