package com.potato.study.leetcodecn.p00359.t001;
 ...
 * bool shouldPrintMessage(int timestamp, string message) 如果这条消息 message 在给定的时间戳 timestamp 应该被打印出来，则返回 true
 * ，否则请返回 false 。
 *  
 *
 * 示例：
...
public class Logger {
    ...
    private Map<String, Integer> msgCanPrintTimeMap;

    public Logger() {
        this.msgCanPrintTimeMap = new HashMap<>();
    }

    public boolean shouldPrintMessage(int timestamp, String message) {
        if (!msgCanPrintTimeMap.containsKey(message)) {
            msgCanPrintTimeMap.put(message, timestamp + 10);
            return true;
        }
        int canPrintTime = msgCanPrintTimeMap.get(message);
