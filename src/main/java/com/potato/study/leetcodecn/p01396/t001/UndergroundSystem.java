package com.potato.study.leetcodecn.p01396.t001;
...
public class UndergroundSystem {
    ...
    // 某个用户 id 进站时间和站名
    private Map<Integer, PassengerInfo> passengerInfoMap;
    // key stationName1_stationName2 int[] 第一个是总时间 第二个 是 人数
    private Map<String, int[]> stationStatisticMap;

    public UndergroundSystem() {
        this.passengerInfoMap = new HashMap<>();
        this.stationStatisticMap = new HashMap<>();
    }

    public void checkIn(int id, String stationName, int t) {
        passengerInfoMap.put(id, new PassengerInfo(t, stationName));
    }

    public void checkOut(int id, String stationName, int t) {
        PassengerInfo remove = passengerInfoMap.remove(id);

        String key = remove.stationName + "_" + stationName;

        int[] ints = stationStatisticMap.get(key);
        if (ints == null) {
            ints = new int[2];
            stationStatisticMap.put(key, ints);
        }
        // 总时间
        ints[0] += (t - remove.startTime);
        // 总人数
        ints[1]++;
    }
    ...
    class PassengerInfo {
        ...
        public PassengerInfo(int startTime, String stationName) {
